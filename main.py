...
def eliminar_usuario(id_usuario: str):
	...
	return {"mensaje":"Usuario no puede ser eliminado por tener un proyecto asignado"}
...
async def crear_proyecto(proyecto: Proyectos):
	try:
		...
		raise HTTPException(status_code=404, detail="Usuario no encontrado, no se puede crear el proyecto con un usuario invalido.")
	try:
...
async def obtener_proyectos_usuario(id_usuario: str= Path(...,description="ID del usuario a recuperar sus proyectos")):
	...
	proyectos_list = list(proyectos)
	if not proyectos_list:
		raise HTTPException(status_code=404, detail="No se encontraron proyectos para el usuario especificado.")
...
def actualizar_proyecto(proyecto_id: str, actualizar_proyecto: Proyectos):
	try:
		...
		raise HTTPException(status_code=404, detail="Usuario no encontrado, no se puede actualizar el proyecto con un usuario invalido.")
	try:
